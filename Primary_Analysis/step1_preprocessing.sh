...
function runSteps () {
    ...
    sed "s#TEST_SAMPLE#${4}#g;s#PROJECT#${2}#g;s#SUBJECT#${3}#g;s#DATA_PATH#$ENV{data_path}#g;s#TRIM##g" run_${1}.sh |bsub -J ${1}_${2}_${3}_${4}
...
runSteps fastqc $project $subject $sample

#2. Trimgalore to remove adapter and low quality reads
runSteps trim_galore $project $subject $sample
...
runSteps fastq_to_uBAM $project $subject $sample

#5. BWA MEM Alignment
runSteps bwa_mem $project $subject $sample

#6. Collect Quality Metrics
#6.1 Qualimap to gather both exome and rnaseq qc metrics
runSteps qualimap $project $subject $sample

#6.2 HsMetrics to collect hybrid selection metrics for WES data
runSteps CollectHsMetrics $project $subject $sample

#6.3 MultipleMetrics
runSteps CollectQualityMetrics $project $subject $sample

#7. MarkDuplicate
runSteps markduplicate $project $subject $sample

#8. Estimate Base Quality
runSteps BaseRecalibrator $project $subject $sample

#9. Apply BQSR
runSteps ApplyBQSR $project $subject $sample
